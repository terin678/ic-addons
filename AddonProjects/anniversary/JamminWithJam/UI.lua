...
local STYLE = ICUI:Style("JamminWithJam", {
    rowHeight = ROW_H,
    headerHeight = 18,
...
function UI.Create()
    ...
    local f = ICUI:Window("JamminWithJamFrame", {
        style = STYLE, width = WIDTH, height = HEIGHT,
        title = "JamminWithJam " .. ns.VERSION,
    })
    UI.frame = f

    -- The Sounds page shows a countdown while a cooldown is running, and
    -- nothing else pushes a redraw once a second on its own. Cheap: Refresh
    -- returns immediately whenever the window is not shown.
    if not UI.ticker then UI.ticker = C_Timer.NewTicker(1, UI.Refresh) end
...
-- Opens the window on a named tab. /jam demo uses it.
...
function UI.Refresh()
    ...
    f.status:SetText(string.format("%s  \194\183  %d sounds  \194\183  channel %s",

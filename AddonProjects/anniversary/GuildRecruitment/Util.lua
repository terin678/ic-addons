...
Anyone in the guild can send a message with our prefix, and what they send is
data, never markup and never code. Escapes come off, any surviving pipe goes
(one on its own can still eat a chat row), and the result is cut to a length this
addon chose rather than one the sender did.
]]
...
-- A span of seconds as a list cell shows it. Separate from Freshness because
-- "how long is left" and "how long ago" want the same words but not the same
-- question, and one of them has no timestamp to work from.

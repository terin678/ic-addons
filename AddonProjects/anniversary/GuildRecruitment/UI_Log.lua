...
local KINDS = {
    ...
    { key = "remote", label = "Others" },
    { key = "doc", label = "Message" },
}

local SHOWN = 100

UI.RegisterPage(50, "Log", function(page)
    local function View()
        return ns.db.settings.log
    end

    -- right = -26 so the toolbar ends where the list does. The widths below are
    -- counted, not guessed: six kinds plus Source plus Clear reach x=520 of 674,
    -- which leaves the count line on the right about 150 to live in.
    local bar = UI.Toolbar(page, { top = 0, right = -26 })

    local kindButtons = {}
    for _, kind in ipairs(KINDS) do
        local b = bar:Left(UI.Button(bar, kind.label, 52, 22))
    ...
    local source = bar:Left(UI.Button(bar, "Source: all", 110, 22))
    ...
    local clear = bar:Left(UI.Button(bar, "Clear", 56, 22, { kind = "danger" }))
